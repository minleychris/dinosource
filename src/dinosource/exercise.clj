    ...
    :script ["We can turn a pixel off as well. You'll see that there's a new block on the library, called \"Off\"."
     ...
     {:title "Your own Blink block"
      ...
      :script ["Blinking a pixel is fun, but repeating the same blocks over and over is not. What we really want is to give a name to \"turn this pixel on, then turn it off\", and just say that name when we want it to happen."
               ...
               "Now, we have only created the block. To make it actually execute On and Off, we need to \"execute\" it. Notice that your new Blink block appeared in the library. So drag it from the library into the code section."
     ...
     {:title "Smling and Blinking"
      ...
      :script ["Now we can create blocks to easily repeat tasks, like blinking a pixel. Let's put this to work and make a smiling face that blinks both of its eyes!"
               "So: drag blocks to turn on the pixels at (2, 4) and (2, 6) for the eyes. Then, for the smile, turn on the pixels at (3, 3), (4, 4), (4, 5), (4, 6) and (3, 7)."
               "Finally, create two new blocks (which you can call Blink Left and Blink Right, for example), and then drag them into the code section to make the eyes blink a few times."
     ...
     {:title "A single block to Blink anywhere"
      ...
      :script ["Great, we have a smiley that blinks. But Blink Left and Blink Right are basically the same thing. The only difference is the position where we turn the pixels On and Off. We don't want to create two different blocks for this. We want to create a single block, \"Blink\", and say \"blink here\", \"now blink there\" and so on."
               "We can do this by passing \"parameters\" into the block. Just like we give a position to the \"On\" and \"Off\" blocks, we can tell our own Blink block that it should be given a position, and use it when they are executed."
               "Notice that \"Block\" on the library has changed since the previous exercise. It now has a space for \"parameters\". So, drag this new Block into the code section and give it a name (say, Blink). The parameters, just like for \"On\" and \"Off\", are the coordinates for the position where we want the pixel to blink. You can call them anything, like \"A\" and \"B\", \"First\" and \"Second\". But let's just call them X and Y, the same parameter names that \"On\" uses."
               "Finally, we need to define the body of the block. Drag an \"On\" block into your Blink block. Now, instead of filling it in with numbers, like we did before, we can use the names that we chose as parameters: X and Y. Do the same with the \"Off\" block."
               "Now, our Blink block turns a pixel on at (X, Y) and then turns it off. Let's execute it! You'll see that the Blink block, that was added to the library when you created it, has two spaces for parameters. Drag it into the code section, and fill with a position, like (1, 3). Now, drag it again, and fill in a different position, like (5, 6). When the first Blink block is executed, X will be 1 and Y will be 3, and the pixel at (1, 3) will blink. When the second Blink is executed, X will be 5 and Y will be 6, so that pixel will now blink."
